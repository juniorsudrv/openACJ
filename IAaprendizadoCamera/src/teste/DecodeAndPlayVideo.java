 ...
 * @author junio
 */
import IAAprender.RedesACJWebcam;
...
public class DecodeAndPlayVideo {
    ...
    public RedesACJWebcam red = null;
     ...
     */
    public void playVideo(RedesACJWebcam red, String filename) throws InterruptedException, IOException {
     ...
     * do work.
         ...
         */
        final ImageFrame window = red == null ? ImageFrame.make() : null;

        /**
         * Media playback, like comedy, is all about timing. Here we're going to
         * introduce <b>very very basic</b>
         * timing. This code is deliberately kept simple (i.e. doesn't worry
         * about A/V drift, garbage collection pause time, etc.) because that
         * will quickly make things more complicated.
         *
            ...
            if (packet.getStreamIndex() == videoStreamId) {
                ...
                do {
                    ...
                    if (picture.isComplete()) {
                        image = red == null ? displayVideoAtCorrectTime(streamStartTime, picture,
                                converter, image, window, systemStartTime, systemTimeBase,
                                streamTimebase)
                                : displayVideoAtCorrectTime(streamStartTime, picture,
        ...
        do {
            ...
            if (picture.isComplete()) {
                image = red == null ? displayVideoAtCorrectTime(streamStartTime, picture, converter,
                        image, window, systemStartTime, systemTimeBase, streamTimebase)
                        : displayVideoAtCorrectTime(streamStartTime, picture, converter,
        ...
        playVideo(red, filename);

        // similar with the demuxer, for the windowing system, clean up after yourself.
        // window.dispose();
    }

    public void playVideoWithoutFrame(RedesACJWebcam red, String filename) throws InterruptedException, IOException {
    ...
    private BufferedImage displayVideoAtCorrectTime(long streamStartTime,
        ...
        long streamTimestamp = picture.getTimeStamp() / 30;
        ...
        while (streamTimestamp > ((systemTimestamp - systemStartTime))) {
            Thread.sleep(1);
        ...
        System.out.println("Duracao " + (System.currentTimeMillis() - systemStartTime) / 1000);
    ...
    private BufferedImage displayVideoAtCorrectTime(long streamStartTime,
            ...
            final Rational systemTimeBase, final Rational streamTimebase)
            throws InterruptedException {
        long streamTimestamp = picture.getTimeStamp() / 34;
        // convert streamTimestamp into system units (i.e. nano-seconds)
        // streamTimestamp = systemTimeBase.rescale(streamTimestamp - streamStartTime, streamTimebase);
        // get the current clock time, with our most accurate clock
        long systemTimestamp = System.currentTimeMillis();
        ...
        red.fundoWork = converter.toImage(image, picture).getScaledInstance(red.pracha_camera.getWidth(),
                 red.pracha_camera.getHeight(), Image.SCALE_FAST);
//        red.fundo = 
//               red. toBufferedImage(red.fundo.getScaledInstance(red.pracha_camera.getWidth()
//                       , red.pracha_camera.getHeight(), Image.SCALE_AREA_AVERAGING));  
        red.trabalhando = true;
        red.pracha_camera.repaint();
        // And ask the UI thread to repaint with the new image.

        //  System.out.println("DuracaoX "+(System.currentTimeMillis()-systemStartTime)/1000 );
        while (isStop() || red.trabalhando) {
    ...
    private static void playVideoOld(String filename) throws InterruptedException, IOException {
        /*
     * Start by creating a container object, in this case a demuxer since
     * we are reading, to get video data from.
         */
     ...
     * do work.
        ...
        while (demuxer.read(packet) >= 0) {
            ...
            if (packet.getStreamIndex() == videoStreamId) {
                ...
                do {
                    bytesRead += videoDecoder.decode(picture, packet, offset);
                    if (picture.isComplete()) {
                        image = displayVideoAtCorrectTimeOld(streamStartTime, picture,
                                converter, image, window, systemStartTime, systemTimeBase,
                                streamTimebase);
                    }
                    offset += bytesRead;
        ...
        do {
            videoDecoder.decode(picture, null, 0);
            if (picture.isComplete()) {
                image = displayVideoAtCorrectTimeOld(streamStartTime, picture, converter,
                        image, window, systemStartTime, systemTimeBase, streamTimebase);
            }
        } while (picture.isComplete());
     ...
     * Takes the video picture and displays it at the right time.
     */
    private static BufferedImage displayVideoAtCorrectTimeOld(long streamStartTime,
            final MediaPicture picture, final MediaPictureConverter converter,
            BufferedImage image, final ImageFrame window, long systemStartTime,
            final Rational systemTimeBase, final Rational streamTimebase)
            throws InterruptedException {
//    long streamTimestamp = picture.getTimeStamp();
        long streamTimestamp = picture.getTimeStamp() / 6;
        // convert streamTimestamp into system units (i.e. nano-seconds)
        streamTimestamp = systemTimeBase.rescale(streamTimestamp - streamStartTime, streamTimebase);
        // get the current clock time, with our most accurate clock
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        try {
            ...
            if (cmd.hasOption("version")) {
                ...
                DecodeAndPlayVideo dc = new DecodeAndPlayVideo();

                dc.playVideoOld(fileChooser.getSelectedFile().getAbsolutePath());
            }
